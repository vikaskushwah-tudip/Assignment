...
public class Controller
{
    ...
    public final static int USER_CUSTOMER = 3;
    ...
    private boolean selectionCheck(int selection)
    {
        ...
        switch(userType)
        {
            ...
            break;
            case USER_EMPLOYEE:
            if( selection <= 0 || SCENE_TABLE < selection)
                result = false;
            break;
            case USER_CUSTOMER:
                if( selection <= 0 || SCENE_TABLE < selection)
    ...
    {
        cView.loginView();
        printMessageToView("Login as admin? (Y/N)");
    ...
    // Find user
    //----------------------------------------------------------
    private void loginCheck( boolean isAdmin)
    {
        ...
        //---------search user----------
        Customer customer =cDatabase.findCustomerByID(inputID);
        Staff   rStaff = cDatabase.findStaffByID(inputID);

        if(isAdmin)   searchClassName = "Manager";
        ...
        if( rStaff != null)//User data is found
        {
            ...
            if( rStaff.getClass().getName().equalsIgnoreCase(searchClassName))
            {
                if(rStaff.getPassword().equals(iuputPassword))
                {
                    ...
                    if(isAdmin)   userType = USER_MANAGER;
    ...
    // Choose edit mode (1:Add 2:Update 3:Delete)
    //----------------------------------------------------------
    private void chooseEditTableMode()
    {
        String  key;
        int     inputNumber = 0;
    ...
    //----------------------------------------------------------
    // Make and check new ID (used by addTable method only)
    //----------------------------------------------------------
    private int generateId()
    {
        int newID = 0;
        ...
        printMessageToView("Choose Table ID for new Table:");
        key = cView.userInput();
